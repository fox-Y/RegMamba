...
class BaseDataSets_Synapse(Dataset):
    ...
    def __getitem__(self, idx):
        case = self.sample_list[idx]
        if self.split == "train":
            # Synapse数据在 Synapse/train_npz/ 目录下
            h5f = np.load(self._base_dir + "/Synapse/train_npz/{}.npz".format(case))
        else:
            if self.split == "val":
                # Synapse测试数据在 Synapse/test_vol_h5/ 目录下
...
class BaseDataSets_CHAOS(Dataset):
    def __init__(
        self,
        base_dir=None,
        split="train",
    ...
    def __getitem__(self, idx):
        case = self.sample_list[idx]
        if self.split == "train":
            # CHAOS训练数据在 train_slices/ 目录下
            h5f = h5py.File(self._base_dir + "/train_slices/{}.h5".format(case), "r")
        else:
            # CHAOS测试/验证数据在 test_vol_h5/ 目录下
            h5f = h5py.File(self._base_dir + "/test_vol_h5/{}.h5".format(case), "r")
        image = h5f["image"][:]
        label = h5f["label"][:]
        sample = {"image": image, "label": label}
        if self.split == "train":
