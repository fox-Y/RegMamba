...
# Register mechanism parameters
_C.MODEL.USE_REGISTER = True  # 是否使用 register 机制
_C.MODEL.NUM_CLS_TOKENS = 6  # Register token 数量（从12降到6以提升性能）
_C.MODEL.REGISTER_VERSION = 'enhanced_gate'  # Register 版本: 
                                              # 'cross_attn'(方案A/V3), 
                                              # 'pooled'(方案D/V2), 
                                              # 'enhanced_gate'(方案V4 - 最优融合)
...
def update_config(config, args):
    ...
    # merge from specific arguments (使用hasattr检查属性是否存在)
